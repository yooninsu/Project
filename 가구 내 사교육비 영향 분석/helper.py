...
def scailing(df, yname):
    ...
    if DataFrame(y_train).dtypes.values == 'category': #카테고리인 경우 표준화를 거치지 않음
        y_train_std_df = y_train
    else:
...
def my_logit(data, y, x):
    ...
    for i, v in enumerate(summary.table[1].data):
        if i == 0:
            continue
